...
public class AddTopicsTest {
	...
	public void addOfferTopicsTest() {
		Category category = categoryRepository.findAll().stream().filter(e -> e.getFieldTypes().size() > 0).findFirst()
				.orElse(null);
	...
	private void addTopic(Category category, Advertiser advertiser, List<FieldType> fieldTypes, String searchOrOffer) {
		...
		for (FieldType fieldType : fieldTypes) {
			...
			case CHOOSE_MULTI_OPTION: {
				topicValue.setValueNum(Double.valueOf(ThreadLocalRandom.current().nextInt(
						fieldType.getMinValue() != null ? fieldType.getMinValue() : 0,
						fieldType.getMaxValue() != null ? fieldType.getMaxValue() + 1 : Integer.MAX_VALUE)));
				break;
			}
			...
			case EMAIL: {
				topicValue.setValueVarchar(
						String.format("%s@%s.%s", randomWord(10, 20), randomWord(5, 10), randomWord(3)));
			...
			case PRICE: {
				topicValue.setValueNum(ThreadLocalRandom.current().nextDouble(
						fieldType.getMinValue() != null ? fieldType.getMinValue() : 0,
						fieldType.getMaxValue() != null ? fieldType.getMaxValue() + 1 : Integer.MAX_VALUE));
				break;
			}
